...
$stmt = $conn->prepare("SELECT * FROM carts WHERE account = ?");
...
if (!$stmt->num_rows()) {
  $stmt = $conn->prepare("INSERT INTO carts (account, products) VALUES (? ,?)");
  ...
  $stmt = $conn->prepare("UPDATE carts SET products = ? WHERE account = ?");
